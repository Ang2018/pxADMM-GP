function [t, z, history] = clasADMM(rho, alpha, inihyp, covfunc, localdata)
...
for k = 1:MAX_ITER
    ...
    spmd (N)
        ...
        for kk = 1:MAX_ITER
            ...
            [nlml, grad] = getNlmlGrad(theta, covfunc, localdata.x, localdata.y);
            objgrad = grad + beta + rho * (theta - z);
            theta = thetaold - alpha * objgrad;
            
            theta_norm = norm(theta - thetaold);
            
            if theta_norm < INT_TOL
                break;
            end
